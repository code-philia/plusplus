...
def _render_glue(
    ...
    sources[database_path] = _render_database_client(database_imports)
...
def _render_database_client(imports: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            *_render_imports(imports),
            ...
            '  response.status(503).json({ error: String(error?.message ?? error) });',
            "});",
            "",
        ]
    )
...
def _build_backend_manifest(
    ...
    exports_by_path.setdefault("backend/src/runtime/errors.ts", set()).add("NotImplementedError")
